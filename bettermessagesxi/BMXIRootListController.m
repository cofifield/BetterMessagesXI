#include "BMXIRootListController.h"
#import <CepheiPrefs/HBTwitterCell.h>
#import <spawn.h>

@implementation BMXIRootListController
...
-(void)respring {
	// Kill Messages App
	pid_t mpid;
	const char* margs[] = { "killall", "-HUP", "MobileSMS", NULL };
	posix_spawn(&mpid, "/usr/bin/killall", NULL, NULL, (char* const*)margs, NULL);

	// Respring - Welcome to iOS 11
	pid_t pid;
...
-(void)paypalco {
   ...
   [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://www.paypal.me/cofifield"]];
}
...
-(void)github {
   ...
   [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://github.com/cofifield/BetterMessagesXI"]];
}
- (void)loadView {
	[super loadView];

	UIImage *credits = [UIImage imageNamed:@"credits.png"];
...
- (void)loadView {
	...
	UIImage *hide = [UIImage imageNamed:@"hide.png"];
...
- (void)loadView {
	...
	UIImage *Misc = [UIImage imageNamed:@"Misc.png"];
...
- (void)loadView {
	...
	UIImage *theme = [UIImage imageNamed:@"theme.png"];
